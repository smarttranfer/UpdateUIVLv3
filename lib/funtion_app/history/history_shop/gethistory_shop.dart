...
class gethistory_shop{
  ...
  static Future<void> gethistory(String token) async {
    try{
      constant_history_shop.listhistory_customer_shop.clear();
      final prefs = await SharedPreferences.getInstance();
      String DC_address = await prefs.getString("DC_adress").toString();
      var headers = {
        'Authorization': 'Bearer ${token}'
      };
      var request = http.Request('GET', Uri.parse('${DC_address}/log/shop?shop_id=-1&user_id=-1&sort=desc&status=all'));
      ...
      if (json.decode(constant_history_shop.Jsondata)["status"].toString() == "200") {
        for(var history in json.decode(constant_history_shop.Jsondata)["data"]){
          ...
          h_Shop.customer_id = history["shop_id"];
